...
const useResponsiveDimension = (responsive: boolean, config: ResolvedConfig, key: 'height' | 'width') => {
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      if (w <= 480 && config[`mobile${suffix}`]) v = config[`mobile${suffix}`] as string
      else if (w <= 768 && config[`tablet${suffix}`]) v = config[`tablet${suffix}`] as string
      else if (w <= 1024 && config[`desktop${suffix}`]) v = config[`desktop${suffix}`] as string
      setValue(v)
    }
