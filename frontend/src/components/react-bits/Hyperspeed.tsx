...
/* eslint-disable @typescript-eslint/no-explicit-any -- vendored vanilla three.js engine; internals below are
   intentionally untyped (see comment above HyperspeedApp), only the public component API is strictly typed */
...
const DEFAULT_EFFECT_OPTIONS: Required<Omit<HyperspeedEffectOptions, 'onSpeedUp' | 'onSlowDown'>> & {
  onSpeedUp: (ev?: Event) => void
  onSlowDown: (ev?: Event) => void
} = {
  onSpeedUp: () => {},
...
// The internal engine below is a vanilla three.js "mini app" (not idiomatic
// React) ported as-is from the original source. Its classes are intentionally
// loosely typed (`any`) rather than fully modeled — they're an implementation
// detail behind the typed `Hyperspeed` component below, not part of the
// public API surface.
...
class CarLights {
  ...
  constructor(webgl: any, options: any, colors: number | number[], speed: number | [number, number], fade: THREE.Vector2) {
    this.webgl = webgl
...
class HyperspeedApp {
  ...
  // version, so there's nothing left to preload before init() runs.
  loadAssets() {
